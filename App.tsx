...
import { fetchTrendingNews, generateBlogPost } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleStartPosting = async () => {
    ...
    try {
      ...
      setPostingResult({
        success: true,
        message: '블로그 포스트가 생성되었습니다. 네이버 블로그로 이동하여 수동으로 붙여넣기 하세요.',
      ...
      // Step 4: 네이버 블로그 글쓰기 페이지 새 탭에서 열기
      console.log('🌐 Step 4: 네이버 블로그 글쓰기 페이지 열기...');
      const blogUrl = `https://blog.naver.com/${naverId}/postwrite`;
      window.open(blogUrl, '_blank');

      // TODO: 실제 자동화를 위해서는 Playwright 백엔드 API가 필요합니다.
      // 현재는 사용자가 수동으로 복사-붙여넣기 할 수 있도록 미리보기를 제공합니다.
      ...
      // 보안: 계정 정보 메모리에서 제거
      console.log('🔒 보안 청소: 프론트엔드 계정 정보 파기');
    }
  };
             ...
             {activeView === 'news-blog' ? (
                <div className="grid grid-cols-1 lg:grid-cols-1 gap-12 max-w-4xl mx-auto">
                    ...
                    <section className="bg-[#1a1a1a] rounded-3xl p-10 border border-gray-800 shadow-2xl shadow-black/40">
                      ...
                      <textarea value={thoughts} onChange={e => setThoughts(e.target.value)} rows={6} placeholder="뉴스에 대한 나의 생각이나 의견을 적어주세요..." className="w-full px-6 py-5 bg-[#0f0f0f] border border-gray-800 text-white placeholder-gray-600 rounded-2xl mb-10 outline-none font-bold resize-none" />
